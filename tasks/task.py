...
class Task:
    ...
    # DEBUGGING --> LOOK AT STUFF 
    def TEST_validate_on(self, set: torch.utils.data.Dataset, loader: torch.utils.data.DataLoader) -> Tuple[Any, float]:
        self.model.eval()
    ...
    def validate_on_names(self, name_it: Iterable[str]) -> Dict[str, Any]:
        ...
        for name in name_it:
            ...
            # Removed for python notebook prints...
    ...
    def train(self):
        ...
        for d in self.train_loader:
            ...
            if self.helper.state.iter % 500 == 0:
                print(f"Now batch number: {self.helper.state.iter}")
    ...
    # New modified
    def train_step(self, data: Dict[str, torch.Tensor], mask_class_id: int = None) -> Result:
        with self.forward_time_meter:
            data = self.helper.to_device(data)

            self.optimizer.zero_grad()

            if mask_class_id != None and mask_class_id != -1:
                res = self.model_interface(data, mask_class_id)
            else:
                res = self.model_interface(data)
    ...
    # INFO-NOTE: 
    # THIS METHOD IS OVERRIDDEN THROUGH INHERITANCE FOR EACH SPECIFIC TASK

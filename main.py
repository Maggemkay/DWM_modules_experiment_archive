...
def load_weights(helper: framework.helpers.TrainingHelper, task: tasks.Task):
    ...
    pretrained = os.path.join(os.path.expanduser(helper.opt.restore_pretrained), "model.pth")
...
# "data" here is the state-dict...
def load_weights_2(data, task: tasks.Task):
    ver = data.get("VERSION", 0)
    if ver == 0:
        task.model.model_parameters.load_state_dict(data)
    else:
...
def main():
    ...
    ############################################
    ############################################
    ## Just parses a bunch of crap up untill this point

...
class CIFAR(ImageClassificationDataset):
    ...
    def load_data(self, cache_dir: str, set: str) -> ImageSet:
        ...
        imageset = ImageSet(data["data"].reshape(-1, 3, 32, 32), data[self.LABELS_NAME])

        return imageset
        # return ImageSet(data["data"].reshape(-1, 3, 32, 32), data[self.LABELS_NAME])

    # # NOTE: CUSTOM "load_data"
    # def load_data(self, set: str) -> ImageSet:

    #     if set == "valid": # <--- Could also be "test", idk this codebase makes no sense.
    #         test_dataset = torchvision.datasets.CIFAR10("data/", train=False, download=True, transform=torchvision.transforms.ToTensor())
    #         # data = np.transpose(test_dataset.data, (0, 3, 1, 2)).astype(np.float32) / 255.0  # (N, C, H, W) and scaled
    #         data = np.transpose(test_dataset.data, (0, 3, 1, 2)).astype(np.float32)  # (N, C, H, W)
    #         labels = np.array(test_dataset.targets, dtype=np.uint8)
    #         type(self).class_names = test_dataset.classes
    #         type(self).n_classes = len(self.class_names)
    #     else:
    #         train_dataset = torchvision.datasets.CIFAR10("data/", train=True, download=True, transform=torchvision.transforms.ToTensor())
    #         # data = np.transpose(train_dataset.data, (0, 3, 1, 2)).astype(np.float32) / 255.0  # (N, C, H, W) and scaled
    #         data = np.transpose(train_dataset.data, (0, 3, 1, 2)).astype(np.float32)  # (N, C, H, W)
    #         labels = np.array(train_dataset.targets, dtype=np.uint8)
    #         type(self).class_names = train_dataset.classes
    #         type(self).n_classes = len(self.class_names)

        return ImageSet(data, labels)

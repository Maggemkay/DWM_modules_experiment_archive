# Code experiment archive

This is a code archive of our experiments which were based on the work by Csórdas et al. (https://github.com/RobertCsordas/modules).

We made quite a lot of changes to how you interact with the code to instead interact with it through python notebooks and python-files locally or when run on a server. This is made for experimental use only, and should not be seen as modifications to modernize the original repository.

Changes are spread across most of the code-base; Duplication of code, comments, removed code, etc... We give the code "as-is", and we have not made any clean-up of the code.

If you wish to investigate and run the "new" code as we have, the main changes can be found here:
* ```updated_new_reqs.sh```: New requirements roughly in installation order.

* ```main_CIFAR10_ONE_CLASS_MASKS_final_1v1_masks.ipynb```: Final used experiment with all changes at the start to interact with the code through a notebook.
